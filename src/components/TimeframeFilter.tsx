...
// Guarda CHAVES; o texto e resolvido no componente (useI18n).
export const TIMEFRAME_OPTIONS: Array<{ value: Timeframe; key: TKey }> = [
  { value: "ALL", key: "timeframe.all" },
  { value: "7_DAYS", key: "timeframe.last7" },
  { value: "30_DAYS", key: "timeframe.last30" },
  { value: "90_DAYS", key: "timeframe.last90" },
  { value: "THIS_MONTH", key: "timeframe.thisMonth" },
  { value: "THIS_YEAR", key: "timeframe.thisYear" },
  { value: "CUSTOM", key: "timeframe.customRange" },
];
...
// O formato da data segue o idioma; sem data escolhida mostra o gabarito.
const formatDateKey = (dateKey: string, locale: string, t: TFn) => {
  const date = fromLocalDateKey(dateKey);
  return date ? new Intl.DateTimeFormat(locale).format(date) : t("timeframe.datePlaceholder");
};
...
export default function TimeframeFilter({ value, onChange, className = "" }: TimeframeFilterProps) {
  ...
  // Nomes curtos dos dias, comecando a segunda-feira, vindos do Intl em vez de
  // uma lista fixa em portugues.
  const weekdayLabels = useMemo(() => {
    const monday = new Date(2024, 0, 1); // 1 de janeiro de 2024 foi uma segunda
    return Array.from({ length: 7 }, (_, index) => {
      const day = new Date(monday);
      day.setDate(monday.getDate() + index);
      return formatDate(day, { weekday: "short" }).replace(".", "");
    });
  }, [formatDate]);
      ...
      <AnimatePresence>
        {value.timeframe === "CUSTOM" && isCustomRangeOpen && (
          <motion.div
            ...
            role="dialog"
aria-label={t("timeframe.pickerAria")}
          >
            <div className="mb-3 flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-semibold text-zinc-800 dark:text-zinc-100">{t("timeframe.customRange")}</p>
                <p className="text-[10px] text-zinc-400 dark:text-zinc-500">{t("timeframe.rangeHint")}</p>
              </div>
            </div>
                ...
                return (
                  <div key={endpoint} className="space-y-1.5">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
                      {t(endpoint === "start" ? "timeframe.startDate" : "timeframe.endDate")}
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                    ...
                    >
                      <span>{formatDateKey(dateKey, locale, t)}</span>
            ...
            <div className="mt-3 rounded-sm border border-zinc-100 bg-zinc-50/70 p-2 dark:border-zinc-800 dark:bg-zinc-950/40">
              <div className="mb-2 flex items-center justify-between px-1">
                ...
                <button type="button" onClick={() => setCalendarMonth(current => new Date(current.getFullYear(), current.getMonth() + 1, 1))} className="flex h-8 w-8 items-center justify-center rounded-sm text-zinc-400 transition-colors hover:bg-white hover:text-emerald-600 dark:hover:bg-zinc-800 dark:hover:text-emerald-300 cursor-pointer" aria-label={t("timeframe.nextMonth")}>
                  <ChevronRight size={15} />
                </button>
              </div>
              <div className="mb-1 grid grid-cols-7">
                {weekdayLabels.map((day, index) => <span key={index} className="flex h-6 items-center justify-center text-[9px] font-bold uppercase text-zinc-400 dark:text-zinc-600">{day}</span>)}
              </div>
